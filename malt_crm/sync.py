...
class MaltSyncService:
    ...
    def sync_conversations(self) -> SyncReport:
        ...
        LOGGER.info("Sync conversations: fetching inbox feed")
        ...
        LOGGER.info(
            "Sync conversations: fetched %s conversations and %s opportunities",
            len(conversations),
            len(opportunities),
        )
        ...
        with self.session_factory() as session:
            ...
            session.commit()
        LOGGER.info("Sync conversations: persisted successfully")
    ...
    def sync_messages(self, conversation_id: str | None = None) -> SyncReport:
        ...
        if not conversation_ids:
            LOGGER.info("Sync messages: no conversations to process")
            return SyncReport()

        total_messages = 0
        LOGGER.info("Sync messages: fetching messages for %s conversations", len(conversation_ids))
        with self.session_factory() as session:
            for current_id in conversation_ids:
                ...
                LOGGER.info("Sync messages: %s -> %s messages", current_id, len(messages))
                for message in messages:
                    upsert_message(session, message)
                total_messages += len(messages)
            session.commit()
        LOGGER.info("Sync messages: persisted %s messages", total_messages)
