"""curl_cffi-based Malt API client built around Malt session cookies."""
...
class MaltAPIClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.session = session or curl_requests.Session(impersonate="chrome131")
    ...
    def from_cookies(
        ...
        *,
        ...
        cookies_json_path: Path | str | None = None,
    ...
    def _request_json(
        ...
        *,
        ...
        LOGGER.info("Malt API request: %s %s params=%s", method, path, params or {})
        try:
            ...
            LOGGER.error("Malt API transport error: %s %s -> %s", method, path, exc)
            raise MaltAPIError(f"{method} {url} failed before response: {exc}") from exc
        LOGGER.info("Malt API response: %s %s -> %s", method, path, response.status_code)
        try:
            response.raise_for_status()
        except curl_requests.RequestsError as exc:
            body_preview = response.text[:500]
            LOGGER.error(
                "Malt API failure: %s %s -> %s body=%s",
                method,
                path,
    ...
    def get_messages(
        ...
        *,
        ...
        wait_for_indexation: bool | None = None,
    ) -> list[Message]:
